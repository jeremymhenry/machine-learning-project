...
from MachineLearningStocks.utils import status_calc
...
def backtest():
    ...
    print(f"Precision score: {metrics.precision_score(y_test, y_pred_rf): .2f}")
    print(f"Recall score: {metrics.recall_score(y_test, y_pred_rf): .2f}\n")
    #print(classification_report(y_test, y_pred_rf))

    # Generate RF Confusion Matrix
    cm = confusion_matrix(y_target=y_test, y_predicted=y_pred_rf, binary=False)
    ...
    print(f"Accuracy score: {knn.score(X_test, y_test): .2f}")
    print(f"Precision score: {metrics.precision_score(y_test, y_pred_knn): .2f}")
    print(f"Recall score: {metrics.recall_score(y_test, y_pred_knn): .2f}")
    # print(classification_report(y_test, y_pred_knn))

    # Generate kNN Confusion Matrix
    cm = confusion_matrix(y_target=y_test, y_predicted=y_pred_knn, binary=False)
    fig, ax = plot_confusion_matrix(conf_mat=cm)
    ...
    num_positive_predictions = sum(y_pred_rf)
    ...
    stock_returns = 1 + z_test[y_pred_rf, 0] / 100
    market_returns = 1 + z_test[y_pred_rf, 1] / 100
